...
export async function commit ({ accessToken, owner, repo, files, message }: CommitProps): Promise<UpdateReferenceResponse> {
  ...
  const commit = await createCommit({ owner, repo, accessToken, parentSha: reference.object.sha, treeSha: tree.sha, message })
